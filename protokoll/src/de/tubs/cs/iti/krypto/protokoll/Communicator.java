...
public class Communicator {
	 ...
	 * eine Socketverbindung zum Server aufgebaut, zwei Streams zur
	 * Kommunikation initialisiert und die fuer die Kommunikation notwendigen
	 * Variablen Spielname Minimal/Maximalspielerzahl initialisiert. Danach
	 * werden diese Daten an den Server uebermittelt, der dann die entsprechende
	 * Spielernummer generiert und zurueckliefert.
	 * 
	...
	public String receive() {
		...
		try {
			while (s.equals("")) { // Aufruf soll blockieren bis Daten empfangen
									// sind
				if (in.ready()) { // zuerst L"ange der Daten lesen, dann Daten
									// selbst
			 ...
			 * 
			 * // if there is more make a newline if (in.ready())
			 * result.println(); } // get the read string result.flush();
			 * w.flush(); s = w.toString(); }
			 */
			...
			if (check == 0) {
				System.out
						.println("communicator: error! connection closed; reason: "
								+ s);
			...
			System.out
					.println("communicator: error! connection closed by server");
	 ...
	 * den Wert "false". Bei erfolgreicher Anmeldung und Start des Spiels
	 * liefert sie "true" zurueck.
	 * 
	 * @exception IOException
		...
		while (!running) {
			...
			if (msg.equals("exit")) {
				try { // falls der Client beenden Button gedrueckt wurde
					...
					System.out
							.println("communicator: error while closing connection");
					return false;
				}
			...
			if (msg.equals("go")) {
				...
				try {
					...
					System.out.println("communicator: number of players "
							+ playerNumber);
