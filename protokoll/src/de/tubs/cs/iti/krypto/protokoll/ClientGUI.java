...
public class ClientGUI extends Frame implements Runnable {
	...
	void connectToServerActionPerformed(ActionEvent e) {
		...
		addHistoryItem(oldServers, "serverNameHistory.txt",
				serverName.getText());
		...
		if (!(client.loadGame(theGameClass.getText()))) {
			...
			addHistoryItem(oldGames, "protocolHistory.txt",
					theGameClass.getText());
		}
	...
	/**
	 * Setzt den Ausgewaehlten Namen in das Textfeld und ruft
	 * "gameNameChanged()" auf.
	 * 
	 * @param ItemEvent
	 *            e ItemEvent
	 ...
	 * 
	 * @param choiceHistory
	 *            Choice-Objekt, dem die History-Eintraege zugewiesen werden
	 *            sollen.
	 * @param historyFile
	...
	void initHistory(Choice choiceHistory, String historyFile) {
		...
		try {
			...
			BufferedReader history = new BufferedReader(new FileReader(
					historyFile));
		...
		// Wenn noch keine History-Datei gefunden wurde, neue leere Datei
		// anlegen.
	 ...
	 * 
	 * @param choiceHistory
	 *            Choice-Objekt, dem die History-Eintraegezugewiesen werden
	 *            sollen.
	 * @param historyFile
	 *            Name der Datei die die History-Eintraege speichert.
	 * @param newItem
	 ...
	 * output. <br>
	 * Wenn der Client bereits am Server angemeldet ist wird die Spielernummer
	 * vor dem Text mitausgegeben. Wenn ein leerer String uebergeben wird, wird
	 * nur die Spielernummer erneuert.
	 * 
	 * @param text
	...
	void printOutput(String text) {
		...
		if (client.getCommunicator() != null) {
			preText = "Spieler " + (client.getCommunicator().myNumber() + 1)
					+ "(Anzahl der Spieler :"
					+ client.getCommunicator().playerNumber() + ") : ";
		} else
			preText = "";

		if (text.equals("")) {

			text = output.getText().substring(
					output.getText().indexOf(": ") + 2);
