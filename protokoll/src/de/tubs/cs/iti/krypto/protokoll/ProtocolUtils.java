...
public class ProtocolUtils extends BigIntegerUtil {
	...
	/**
	 * See ALGORITHMS 5.2.1. Calculate the square root of <code>val</code>
	 * modulo <code>modulus</code>. <code>modulus</code> must be a prime number,
	 * and <code>val</code> a quadratic remainder. Then, an x out of [1, ...,
	...
	public static BigInteger sqrtMod(BigInteger val, BigInteger modulus) {
		...
		while (K[n] != 0) {
			exp = ONE.shiftLeft((lm[0].subtract(BigInteger.valueOf(K[n])))
					.intValue());
	...
	/*
	 * Diese Routine ist ein Teil von Algorithmus 5.2.1. Sie liefert: min { k >=
	 * 0 | y^((2^k)*m) mod p = 1 }
	 */
