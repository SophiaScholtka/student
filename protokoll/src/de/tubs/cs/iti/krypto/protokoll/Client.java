...
public class Client implements IClient {
	...
	public boolean loadGame(String protocolName) {
		...
		try {
			...
			myGUI.printOutput("Protokoll-Klasse " + protocolName
					+ " nicht gefunden!");
			System.err.print("java.lang.ClassNotFoundException : "
					+ protocolName);
	...
	/**
	 * Prueft, ob die Spieleranzahl die Mindestanzahl an Spielern erreicht hat
	 * und aktiviert den Button "Spiel Starten".
	 * 
	...
	/**
	 * Besorgt sich zunaechst die spielrelevanten Daten von seinem
	 * Spielprotokoll. Danach erzeugt sie ein neues Objekt vom Typ Communicator
	 * und uebergibt diesem die Spieldaten sowie den gewuenschten
	 * Servernamen/Port.<br>
	 ...
	 * verknuepfen.
	 * 
	 * @todo Java.lang.InstantiationException abfangen bei EIngabe von z.B.
	 *       Client
	 */
	public void connect() {
		...
		try {
			myCom = new Communicator(host, port, shortName, minPlayer,
					maxPlayer, this);
	...
	/**
	 * Ueberprueft, ob Communicator Objekt vorhanden ist, wenn dies der Fall
	 * ist, wird an den Server die Abmelde-Nachricht geschickt und das
	 * Communicator Objekt auf null gesetzt.
	 */
	...
	/**
	 * Diese Methode macht das Client Fenster wieder sichtbar, benachrichtigt
	 * den Anwender mittels eines popUpFensters ueber den Abbruch des Spiels
	 * durch einen anderen Spieler und Schliesst den Client danach. Die Methode
	 * wird vom Communicator aufgerufen, wenn ein anderer Spieler das laufende
	 * Spiel abbricht.
	 * 
	 * @param message
	...
	/**
	 * Hauptprogramm: Hier werden die Eingabeparameter untersucht und
	 * verarbeitet und der Client erzeugt.
	 * 
	 * @param args
		...
		if (args.length > 0) {
			for (int i = 0; i < args.length; i++) {
				...
				if ((args[i] = args[i].toLowerCase()).startsWith("-port=")) {
					try {
						port = Integer.parseInt(args[i].substring(args[i]
								.indexOf("=") + 1));
				...
				if (port < 1024 || port > 9999) {
					System.out
							.println("Invalid port number! Using default port.");
					port = defaultPort;
				}
